...
import { Play, Music, Video, FileAudio } from 'lucide-react'; // Added icons for Audio/Video distinction
...
const VideoMotivation = () => {
  // Default to the correct category ID expected by the database
  const [activeTab, setActiveTab] = useState('bhajans-video');
  const [mediaData, setMediaData] = useState<MediaItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Updated tabs to match the IDs used in the Admin Dashboard (Dashboard.tsx)
  const tabs = [
    ...
    <div className="space-y-6">
      ...
      {loading && (
        <div className="flex justify-center items-center py-12">
          ...
          <span className="ml-3 text-lg text-primary">Loading media content...</span>
        </div>
      )}
            ...
            <Card className="col-span-full border border-border">
              <CardContent className="p-12 text-center">
                ...
                <p className="text-muted-foreground mb-4">
                  No audio or video content found for the '{activeTab}' category.
                </p>
                <Button
                  variant="outline"
                  onClick={() => window.location.reload()}
