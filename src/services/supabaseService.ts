/**
 * ============================================================================
 * Supabase Service Layer - IRCA Platform
 * ============================================================================
...
export async function getIRCACenterDetails(centerId: string): Promise<IRCACenterDetails | null> {
  ...
  if (!centerId || centerId === 'null' || centerId === 'undefined') {
    console.warn('Invalid centerId provided to getIRCACenterDetails:', centerId);
    return null;
  }
...
export async function getCenterByIdFromDB(id: string): Promise<any> {
  ...
  if (!id || id === 'null' || id === 'undefined' || id === '') {
    console.warn('Invalid id provided to getCenterByIdFromDB:', id);
    return null;
  }

  // Check if this looks like an IRCA center_id (from ircacenter_details table)
  // IRCA center_ids are typically UUIDs that should be checked in ircacenter_details first
  try {
    ...
    console.log('Not an IRCA center detail:', error);
  }

  // If not found in ircacenter_details, try the ircacenters table
  try {
    const { data: ircaCenterData, error: ircaCenterError } = await supabase
      .from('ircacenters')
      .select('*')
      .eq('id', id)
      .single();
    
    if (!ircaCenterError && ircaCenterData) {
      return { ...ircaCenterData, type: 'irca' };
    }
  } catch (error) {
    console.log('Not an IRCA center:', error);
  }

  // Try hospitals only if the ID pattern suggests it might be a hospital
  // (avoid unnecessary API calls for IRCA center IDs)
  try {
  ...
  // Try psychiatrists only as last resort
  try {
...
export async function getIRCACenterIdMap(): Promise<{ [key: string]: string }> {
  // Fetch all IRCA centers with their IDs
  const { data: centers, error } = await supabase
    .from('ircacenters')
  ...
  if (error) {
    console.error('Error fetching IRCA center ID map:', error);
    return {};
  }

  // Create name to center_id mapping
  return (centers || []).reduce((acc: any, center: any) => {
    if (center.center_id) {
      acc[center.name] = center.center_id;
    }
    return acc;
  }, {});
}
  ...
  (centers || []).forEach((center: any) => {
    ...
    const partialMatch = (details || []).find((detail: any) => {
      ...
      const centerKeyWords = centerName
        .split(' ')
        .filter(word => word.length > 3 && !['rehabilitation', 'centre', 'center', 'association', 'institute'].includes(word));
      
      // Extract key words from detail title
      const detailKeyWords = detailTitle
        .split(' ')
        .filter(word => word.length > 3 && !['integrated', 'rehabilitation', 'centre', 'center', 'association', 'institute'].includes(word));
      
      // Match if at least 2 unique key words match
      const matches = centerKeyWords.filter(centerWord => 
        detailKeyWords.some(detailWord => 
